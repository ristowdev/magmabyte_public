...
export default function PageBuilder(props: IPageBuilderProps) {
    ...
    const [rememberComponents, setRememberComponents] = useState<Component[]>([]);
    ...
    useEffect(()=>{
        if(template && editData){
            
            setRememberComponents(pageComponents);
            var _tp = editData.page_template === 'home_page' ? 'Home page' : 'Blog post'; 
            console.log(template)
            if(template!=_tp){ 
                setPageComponents([]);
            }else{
                if(rememberComponents.length > 0) {
                    setPageComponents(rememberComponents);
                    returnNewPageContent(null);
                }
            }
        }
